    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        try {
            ...
            if (($cached = $store->get($key)) !== null) {
                return response($cached['body'], $cached['status'], $cached['headers'])->header('X-Cache', 'HIT');
            }
            ...
            try {
                ...
                if ($response->isSuccessful() && ! $response->headers->has('Set-Cookie') && ! $response instanceof StreamedResponse) {
            ...
            return $next($request)->header('X-Cache', 'ERROR');
        }
    }

    private function eligible(Request $request): bool
    {
        if (! config('cache_architecture.enabled') || ! config('cache_architecture.full_page')) {
            return false;
        }
        if (! in_array($request->method(), ['GET', 'HEAD'], true) || $request->user() || $request->expectsJson()) {
            return false;
        }
        if ($request->routeIs('search', 'login', 'register', 'password.*', 'news.legacy', 'news.legacy-root') || $request->is('admin/*', 'livewire/*')) {
            return false;
        }
