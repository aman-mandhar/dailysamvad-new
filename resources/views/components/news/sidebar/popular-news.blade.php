...
<section class="ds-sidebar-widget" data-sidebar-widget="{{ $widget->key }}" aria-labelledby="sidebar-{{ $widget->key }}-heading">
    ...
    <div class="ds-sidebar-widget__body">
        @foreach ($widget->items as $post)<x-news.sidebar.news-item :post="$post" :show-category="$widget->showCategory" :show-date="$widget->showDate" :rank="$loop->iteration" />@endforeach
    </div>
</section>
